                                   ...
                                   int primary)
{
    ...
    if (primary && parent != NULL
        ...
        /*
         * We are creating the primary DRBG but we have a seed source that lives
         * in another provider. DRBGs from our FIPS provider can only be seeded
         * by DRBGs that live inside the FIPS module boundary.
         *
         * If the primary DRBG indicates that it cannot be seeded by a source
         * that lives in another provider, then discard the seed source. This
         * scenerio can happen when both the default and FIPS providers are
         ...
         * provider is active, the seed source fetch fails and the primary is
         * seeded with OS entropy.
         */
        int restricted = 0;
        *p++ = OSSL_PARAM_construct_int(UBUNTU_OSSL_DRBG_PARAM_RESTRICTED_RESEED,
                                        &restricted);
